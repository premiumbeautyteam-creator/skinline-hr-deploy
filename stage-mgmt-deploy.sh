#!/bin/bash
# SkinLine HR CRM — stage management deploy (dynamic pipeline stages: create/edit/delete/reorder)
# Idempotent: ships source+built dist tarball; startup seeds the new `stages` table only if empty.
# Safe for production data.db (additive migration; existing rows and OAuth tokens untouched).
set -e
exec > >(tee /tmp/stage-mgmt-deploy.log) 2>&1

APP_DIR=/home/skinline/skinline-hr-crm
PM2_NAME=skinline-hr-crm
TARBALL_URL=https://raw.githubusercontent.com/premiumbeautyteam-creator/skinline-hr-deploy/main/skinline-stage-mgmt.tar.gz
ENV_FILE="$APP_DIR/.env"

cd "$APP_DIR"

echo "[1/6] Download tarball..."
curl -fsSL "$TARBALL_URL" -o /tmp/stage-mgmt.tar.gz
...
tar -xzf /tmp/stage-mgmt.tar.gz -C "$APP_DIR"
...
HTTP_CODE=$(curl -s -o /tmp/stage-mgmt-health.json -w "%{http_code}" "http://localhost:${PORT}/api/integrations/hh" || echo "000")
echo "  GET /api/integrations/hh -> HTTP ${HTTP_CODE}"
head -c 600 /tmp/stage-mgmt-health.json 2>/dev/null || true
echo ""
if [ "$HTTP_CODE" = "200" ]; then
  echo "=== stage-mgmt deploy OK ==="
else
  echo "=== stage-mgmt deploy finished (verify app: pm2 logs ${PM2_NAME}) ==="
fi
